...
const proceedbtn = document.getElementById("button"); //Used for invoking proceed button function
...
function checkletter() {
  ...
  if (n == -1) {
    ...
    for (
      i = 0;
      i < array1.length;
      i++ //Used to replace '-' with correct letter
    ) {
      if (array1[i] == ch) {
        array2[i] = ch;
      }
    ...
    for (
      i = 0;
      i < mod.length;
      i++ //Used to check the winning condition
    ) {
      if (mod[i] == array2[i]) {
        win++;
      }
